...
ROOTS = tuple(sorted(Path("data/streams").glob("probe_*")))
...
class ProbePipelineSmokeTest(unittest.TestCase):
    def test_read_only_all_probes_and_representation_variants(self):
        before_data = _inventory(Path("data"))
        before_outputs = _inventory(Path("outputs"))
        ...
        self.assertEqual(before_data, _inventory(Path("data")))
        self.assertEqual(before_outputs, _inventory(Path("outputs")))
