...
from ocid_gate_c1_common import (  # noqa: E402
    IOU_GRID,
    GateC1ContractError,
    evaluate_profiles,
...
class GateC1CommonTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.inventory = load_development_inventory(SPEC, REVIEWED)

    def test_exact_frozen_development_inventory(self) -> None:
    ...
    def test_rejects_heldout_unlock_role_mismatch_and_wrong_total(self) -> None:
        baseline = json.loads(SPEC.read_text(encoding="utf-8"))
        for mutate, message in (
            (lambda payload: payload["heldout_lock"].update(predictions_unlocked=True), "locked"),
            ...
            with self.subTest(message=message), tempfile.TemporaryDirectory() as temporary:
                ...
                with self.assertRaisesRegex(GateC1ContractError, message):
    ...
    def test_rejects_manifest_role_mismatch(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with self.assertRaisesRegex(GateC1ContractError, "role mismatch"):
    ...
    def test_rejects_frame_path_escaping_stream_directory(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            manifest_path.write_text(json.dumps(manifest), encoding="utf-8")
            with self.assertRaisesRegex(GateC1ContractError, "escapes reviewed root"):
                load_development_inventory(SPEC, fixture)
    ...
    def test_rejects_missing_or_heldout_prediction_keys(self) -> None:
        with self.assertRaisesRegex(GateC1ContractError, "exactly the development streams"):
    ...
    def test_exact_input_pairs_reject_subset_and_cross_paired_annotations(self) -> None:
        ...
        with self.assertRaisesRegex(GateC1ContractError, "exactly the canonical development"):
            validate_development_input_pairs(
                streams[:-1],
                annotations[:-1],
                benchmark_spec_path=SPEC,
