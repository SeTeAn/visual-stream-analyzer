...
from tools import evaluate_ocid_gate_c2_masked_dinov2 as subject
...
class GateC2MaskedDinoTest(unittest.TestCase):
    ...
    def test_input_contract_rejects_heldout_candidate_and_mask_hash(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            sam = root / "sam.json"
            sam.write_text(json.dumps(self._sam_manifest(selected_path, heldout="unlocked")), encoding="utf-8")
    ...
    def test_evaluation_boundary_rejects_before_loading_annotation(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with mock.patch.object(subject, "load_annotation") as annotations:
                with self.assertRaisesRegex(ValueError, "persisted Gate C2 inference boundary"):
    ...
    def test_mask_pair_slices_have_scores_and_gate_rates(self) -> None:
        ...
        self.assertEqual(report["0.70-0.80"]["gate_pass_count"], 1)
        self.assertIsNotNone(report["0.70-0.80"]["score_distribution"]["mean"])

    def test_c1_mask_iou_keys_include_stream_to_prevent_collisions(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            ...
            values = subject._c1_mask_iou_by_candidate(
    ...
    def test_partial_occlusion_diagnostic_separates_extractor_miss(self) -> None:
        ...
        self.assertEqual(result["adjacent_gate_pass_count"], 1)
    ...
    def test_help_never_runs_inference(self) -> None:
        with mock.patch.object(subject, "run_gate_c2") as runner:
