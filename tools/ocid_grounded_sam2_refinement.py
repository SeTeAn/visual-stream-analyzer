"""Local bbox-prompted SAM2 refinement for Grounding DINO candidates.

The module deliberately knows only RGB pixels and candidate boxes.  It has no
dataset, annotation, benchmark, or ground-truth dependency.  A frame is sent
through SAM2 once with all valid boxes in one prompt batch, then every source
box receives either a usable mask result or an explicit bbox fallback.
"""
