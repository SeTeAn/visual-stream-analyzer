"""Preregistered, development-only Gate C2 DINOv2 representation evaluation.
...
from tools.ocid_gate_c1_common import (
    ...
    GateC1ContractError, load_development_inventory, write_canonical_json_atomic,
)


SCHEMA_VERSION = "ocid-gate-c2-masked-dinov2-evaluation.v1"
INFERENCE_SCHEMA_VERSION = "ocid-gate-c2-dinov2-inference.v1"
EMBEDDING_SCHEMA_VERSION = "ocid-gate-c2-dinov2-embeddings.v1"
...
def _expected_sha256(path: Path, expected: str, label: str) -> str:
    ...
    if actual != normalized:
        raise GateC1ContractError(f"{label} SHA-256 mismatch: expected {normalized}, got {actual}")
    return actual
...
def _under(root: Path, relative: str, label: str) -> Path:
    ...
    if value.is_absolute() or ".." in value.parts:
        raise GateC1ContractError(f"{label} must remain under the SAM2 artifact root")
    resolved = (root / Path(*value.parts)).resolve(strict=True)
    try:
        resolved.relative_to(root.resolve(strict=True))
    except ValueError as error:
        raise GateC1ContractError(f"{label} escapes the SAM2 artifact root") from error
    return resolved
...
def _load_binary_mask(path: Path) -> np.ndarray:
    try:
        ...
        raise GateC1ContractError(f"cannot decode predicted SAM2 mask {path}: {error}") from error
    if values.ndim != 2:
        raise GateC1ContractError("predicted SAM2 mask must be a two-dimensional image")
    if not np.isin(values, (0, 255)).all():
        raise GateC1ContractError("predicted SAM2 mask must be binary 0/255")
...
def _candidate_from_selected(
    ...
    return CandidateRecord(
        envelope=RecordEnvelope(
            record_id=candidate_id, schema_version="ocid-gate-c2-candidate-1.0",
            stream_id=stream_id, producer=_producer("gate_c2_input", "sha256:gate-c2-input"),
        ...
        candidate_source="grounding_dino_gate_c1_1_selected",
...
def load_predicted_candidate_inputs(
    *, inventory: Sequence[DevelopmentStream], selected_candidates_path: Path,
    ...
    """Validate every frozen C1.1 input before decoding RGB or loading DINOv2."""
    if len(inventory) != FROZEN_DEVELOPMENT_STREAM_COUNT:
        raise GateC1ContractError(
            f"Gate C2 requires exactly {FROZEN_DEVELOPMENT_STREAM_COUNT} development streams"
        )
    ...
    if (
        ...
        raise GateC1ContractError("SAM2 manifest must be the development-only persisted C1.1 inference")
    provenance = manifest.get("selected_candidates")
    if not isinstance(provenance, Mapping) or provenance.get("sha256") != selected_digest:
        raise GateC1ContractError("SAM2 manifest does not match the frozen selected candidate set")
    ...
    if not isinstance(by_stream, Mapping) or set(by_stream) != {item.stream_id for item in inventory}:
        raise GateC1ContractError("SAM2 manifest must contain the exact development stream inventory")
    ...
    for stream in inventory:
        ...
        if not isinstance(rows, list) or {row.get("candidate_id") for row in rows if isinstance(row, Mapping)} != set(expected):
            raise GateC1ContractError(f"SAM2 candidate set mismatch for {stream.stream_id}")
        parsed: list[CandidateInput] = []
        for raw in rows:
            if not isinstance(raw, Mapping) or raw.get("status") != "valid":
                raise GateC1ContractError("all selected candidates require valid predicted SAM2 masks")
            candidate_id = str(raw.get("candidate_id"))
            selected_row = expected[candidate_id]
            if raw.get("stream_id") != stream.stream_id or raw.get("frame_id") != selected_row.frame_id:
                raise GateC1ContractError("SAM2 candidate stream/frame identity mismatch")
            ...
            if not isinstance(source_bbox, Mapping) or not _same_bbox(_selected_bbox(source_bbox), selected_row.bbox):
                raise GateC1ContractError("SAM2 candidate bbox does not match selected Grounding DINO bbox")
            frame = frames.get(selected_row.frame_id)
            if frame is None or positions[selected_row.frame_id] != selected_row.frame_index:
                raise GateC1ContractError("selected candidate references an invalid development frame")
            mask_info = raw.get("cleaned_mask")
            if not isinstance(mask_info, Mapping) or not isinstance(mask_info.get("path"), str):
                raise GateC1ContractError("SAM2 candidate is missing a cleaned predicted mask")
            ...
            if full.shape != (frame.image_size.height, frame.image_size.width):
                raise GateC1ContractError("predicted SAM2 mask geometry does not match RGB frame")
            recorded_full_digest = mask_info.get("binary_mask_sha256")
            if not isinstance(recorded_full_digest, str):
                raise GateC1ContractError("cleaned_mask.binary_mask_sha256 is required")
            if binary_mask_sha256(full) != recorded_full_digest:
                raise GateC1ContractError("cleaned predicted SAM2 mask binary hash mismatch")
            # The C1 artifact hash is image-level; it is preserved as provenance.  The
            ...
            if local.shape != (h, w) or not local.any():
                raise GateC1ContractError("predicted SAM2 mask crop must be non-empty and match the exact bbox")
    ...
    if candidate_count != FROZEN_CANDIDATE_COUNT or valid_mask_count != FROZEN_VALID_MASK_COUNT:
        raise GateC1ContractError(
            "Gate C2 requires exactly 1060 selected candidates and 1060 valid predicted masks"
        )
    if sam2_digest != expected_sam2_inference_sha256.casefold():  # defensive audit invariant
        raise GateC1ContractError("SAM2 inference manifest digest changed during validation")
...
def _load_stream(stream_directory: Path):
    return load_decoded_stream(ManifestLoadRequest(
        ...
        producer=_producer("stream_input", "sha256:gate-c2-rgb-input"),
    ))
...
def _validate_provider_frozen(provider: Any) -> None:
    if getattr(provider, "batch_size", None) != FROZEN_BATCH_SIZE:
        raise GateC1ContractError(f"Gate C2 provider batch_size must be {FROZEN_BATCH_SIZE}")
    spec = getattr(provider, "model_spec", None)
    if spec is None:
        raise GateC1ContractError("Gate C2 provider must expose its frozen model_spec")
    if (
        ...
        raise GateC1ContractError("Gate C2 provider does not match frozen ViT-B/14 model configuration")


def _validate_embedding_matrix(matrix: np.ndarray, count: int, label: str) -> np.ndarray:
    value = np.asarray(matrix)
    if value.dtype != np.dtype(np.float32):
        raise GateC1ContractError(f"{label} embeddings must use float32")
    if value.shape != (count, FROZEN_EMBEDDING_DIMENSION):
        raise GateC1ContractError(
            f"{label} embeddings must have shape ({count}, {FROZEN_EMBEDDING_DIMENSION})"
        )
    if not np.isfinite(value).all():
        raise GateC1ContractError(f"{label} embeddings contain non-finite values")
    norms = np.linalg.norm(value.astype(np.float64), axis=1)
    if not np.allclose(norms, 1.0, rtol=0.0, atol=1e-5):
        raise GateC1ContractError(f"{label} embeddings must be approximately unit-normalized")
...
def run_predicted_mask_inference(
    *, inventory: Sequence[DevelopmentStream], inputs: Mapping[str, Sequence[CandidateInput]],
    ...
    if len(inventory) != FROZEN_DEVELOPMENT_STREAM_COUNT or len(ordered) != FROZEN_CANDIDATE_COUNT:
        raise GateC1ContractError("Gate C2 inference requires the exact frozen development inventory")
    ...
    for variant in SUPPORTED_VARIANTS:
        ...
        for start in range(0, len(ordered), FROZEN_BATCH_SIZE):
            ...
            if normalized.shape[0] > FROZEN_BATCH_SIZE:
                raise GateC1ContractError("Gate C2 preprocessing exceeded the frozen provider batch")
    ...
    manifest = {
        ...
        "scope": "development_only_gate_c2", "heldout_access": "none",
...
def _mask_iou_slices(rows: Iterable[dict[str, Any]]) -> dict[str, int]:
    # Actual GT mask IoU is a C1.1 evaluation-only field; Gate C2 preserves the
...
def _pair_report(records: Sequence[PairRankingRecord]) -> dict[str, Any]:
    ...
    return {
        ...
        "gate_0_80": {
            "positive_count": len(positive), "passed_count": sum(value >= FROZEN_VISUAL_GATE for value in positive),
            "pass_rate": None if not positive else sum(value >= FROZEN_VISUAL_GATE for value in positive) / len(positive),
        },
    }
...
def _summarize_mask_slice_scores(values: Mapping[str, Sequence[float]]) -> dict[str, Any]:
    return {
        key: {
            ...
            "gate": FROZEN_VISUAL_GATE,
            "gate_pass_count": sum(score >= FROZEN_VISUAL_GATE for score in values.get(key, ())),
            "gate_pass_rate": None if not values.get(key) else (
                sum(score >= FROZEN_VISUAL_GATE for score in values[key]) / len(values[key])
            ),
        }
...
def _recommendation(variants: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        "rule_id": "gate_c2_preregistered_representation_rule_v1",
...
def _c1_mask_iou_by_candidate(
    inference: Mapping[str, Any],
) -> dict[tuple[str, str], float]:
    """Read C1.1 evaluation data only after Gate C2's persisted boundary.
    ...
    if not report_path.is_file():
        return {}
    report = _object(report_path, "C1.1 mask report")
    ...
    if not isinstance(report_inference, Mapping) or report_inference.get("sha256") != _sha256(manifest_path):
        raise GateC1ContractError("C1.1 mask report does not match the persisted SAM2 inference manifest")
...
def _load_embedding_npz(path: Path, expected_count: int) -> dict[str, np.ndarray]:
    with np.load(path.resolve(strict=True), allow_pickle=False) as archive:
        if set(archive.files) != set(SUPPORTED_VARIANTS):
            raise GateC1ContractError("embedding NPZ must contain exactly the two frozen variants")
        return {
...
def evaluate_persisted_inference(
    *, inventory: Sequence[DevelopmentStream], inputs: Mapping[str, Sequence[CandidateInput]],
    ...
    manifest = _object(inference_manifest_path, "Gate C2 inference manifest")
    if (
        ...
        raise GateC1ContractError("reviewed evaluation requires a persisted Gate C2 inference boundary")
    if (
        ...
        raise GateC1ContractError("persisted Gate C2 inference manifest violates frozen configuration")
    ...
    if len(inventory) != FROZEN_DEVELOPMENT_STREAM_COUNT or len(expected) != FROZEN_CANDIDATE_COUNT:
        raise GateC1ContractError("evaluation requires the exact frozen Gate C2 inventory")
    ...
    c1_mask_iou = _c1_mask_iou_by_candidate(manifest)
    cursor = 0
    ...
    for stream in inventory:
        ...
        for variant in SUPPORTED_VARIANTS:
            ...
            stream_slices = _positive_pair_mask_slice_scores(items, identity, by_id, c1_mask_iou)
            ...
            if stream.stream_id == MARKER_STREAM:
                marker_rows[variant] = _marker_diagnostic(
                    annotation, items, by_id, identity, bbox_iou, c1_mask_iou
                )
    ...
    if occlusion_visibility is None:
        raise GateC1ContractError("frozen partial-occlusion development stream was not evaluated")
    ...
    return {
        ...
        "limitations": [
            ...
            "C1.1 mask-IoU slices are evaluation-only and are never model inputs.",
...
def _marker_diagnostic(
    ...
    if not set(MARKER_LABELS).issubset(reference_labels):
        raise GateC1ContractError("reference frame_0019 must contain reviewed source labels 003 and 009")
    label_identities: dict[int, str] = {}
    for label in MARKER_LABELS:
        values = {str(row["visual_type_id"]) for row in reviewed_rows if row.get("source_label") == label}
        if len(values) != 1:
            raise GateC1ContractError(f"reviewed marker label {label:03d} must map to one physical identity")
    ...
    for frame_id in co_visible_frames:
        for query_label, counterpart_label in ((3, 9), (9, 3)):
            ...
            rank_rows.append({
                ...
                "gate_0_80_passed": cross_score >= FROZEN_VISUAL_GATE,
            })
    ...
    for label in MARKER_LABELS:
        ...
        for gt in gt_rows:
            ...
            rows.append({
                ...
                "c1_1_mask_iou": None if item is None else mask_iou.get(
...
def _source_label_visibility_proxy(
    ...
    if not isinstance(metadata, Mapping) or metadata.get("role") != "development":
        raise GateC1ContractError("partial-occlusion annotation must have the development role")
    source_sequence = metadata.get("source_sequence")
    if not isinstance(source_sequence, str):
        raise GateC1ContractError("partial-occlusion annotation is missing source_sequence")
    ...
    if sequence_path.is_absolute() or ".." in sequence_path.parts:
        raise GateC1ContractError("partial-occlusion source_sequence must be a safe relative path")
    ...
    try:
        ...
        raise GateC1ContractError("partial-occlusion label root escapes the OCID raw root") from error
    ...
    if not isinstance(frame_entries, list):
        raise GateC1ContractError("partial-occlusion stream manifest frames must be a list")
    entries: dict[str, Mapping[str, Any]] = {}
    for row in frame_entries:
        if not isinstance(row, Mapping) or not isinstance(row.get("frame_id"), str):
            raise GateC1ContractError("partial-occlusion stream manifest has a malformed frame")
        entries[row["frame_id"]] = row
    if set(entries) != set(annotation.frame_ids):
        raise GateC1ContractError("partial-occlusion manifest and annotation frame sets differ")
    ...
    for frame_id in annotation.frame_ids:
        ...
        if not isinstance(frame_metadata, Mapping):
            raise GateC1ContractError("partial-occlusion frame metadata is missing")
        filename = frame_metadata.get("ocid_source_filename")
        if not isinstance(filename, str):
            raise GateC1ContractError("partial-occlusion frame source filename is missing")
        ...
        if labels.ndim != 2:
            raise GateC1ContractError("partial-occlusion source label must be two-dimensional")
    ...
    if OCCLUSION_FLAG_FRAME not in areas:
        raise GateC1ContractError("partial-occlusion flagged frame is missing the target observation")
    ...
    if flagged_position == 0:
        raise GateC1ContractError("partial-occlusion flagged frame has no predecessor")
    ...
    if previous.shape != current.shape:
        raise GateC1ContractError("partial-occlusion source label shapes differ")
...
def _partial_occlusion_diagnostic(
    ...
    return {
        ...
        "adjacent_gate_pass_count": sum(
            score >= FROZEN_VISUAL_GATE for score in adjacent_scores
        ),
        "trajectory": rows,
        "claim_boundary": (
...
def run_gate_c2(*, selected_candidates_path: Path, sam2_inference_manifest_path: Path,
    ...
    if (
        ...
        raise GateC1ContractError("expected checkpoint size must be a positive integer")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        run_gate_c2(selected_candidates_path=args.selected_candidates, sam2_inference_manifest_path=args.sam2_inference_manifest,
