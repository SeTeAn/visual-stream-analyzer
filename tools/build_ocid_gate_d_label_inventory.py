"""Freeze the OCID source label files used by Gate D mask evaluation."""
...
DEFAULT_OUTPUT = (
    DEFAULT_PROTOCOL.parent / "ocid_gate_d_source_label_inventory_v1.json"
)


def build_inventory(*, protocol_path: Path, output_path: Path) -> dict[str, Any]:
    protocol = load_protocol_for_source_label_inventory_bootstrap(protocol_path)
    ...
    for item in (*protocol.development, *protocol.heldout):
        ...
        if (
            ...
            raise GateDContractError(f"analysis manifest differs for {item.stream_id}")
        labels: list[dict[str, Any]] = []
        for frame in frames:
            if not isinstance(frame, Mapping):
                raise GateDContractError(f"malformed frame metadata for {item.stream_id}")
            ...
            try:
                ...
                raise GateDContractError("source label path escapes the OCID root") from error
            if not label_path.is_file():
                raise GateDContractError(f"source label is not a file: {label_path}")
            labels.append(
                {
                    "frame_id": frame_id,
    ...
    payload = {
        "schema_version": "ocid-gate-d-source-label-inventory-1.0",
        "benchmark_id": benchmark["benchmark_id"],
        "purpose": "freeze_evaluation_only_source_labels_for_mask_metrics",
        "source_root": "data/ocid/raw/OCID-dataset",
        "benchmark_spec_sha256": _mapping(benchmark, "spec", "benchmark")["sha256"],
        "component_decisions_sha256": _mapping(
            benchmark,
            "component_decisions",
            "benchmark",
        )["sha256"],
...
def _safe_filename(value: str) -> None:
    ...
    if len(path.parts) != 1 or path.name != value or value in {".", ".."}:
        raise GateDContractError("OCID source filename must be one safe path segment")
