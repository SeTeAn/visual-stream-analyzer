"""Shared inventory and aggregation helpers for OCID evaluation.
...
the configured development inventory explicit.
"""
...
class OcidEvaluationError(ValueError):
    """Raised when an OCID evaluation input violates the configured data contract."""
...
def load_development_inventory(spec_path: Path, reviewed_root: Path) -> tuple[DevelopmentStream, ...]:
    """Validate the benchmark and return its development streams only.
    ...
    if not root.is_dir():
        raise OcidEvaluationError(f"reviewed root does not exist: {root}")
    ...
    if set(development_members).intersection(heldout_members):
        raise OcidEvaluationError("a stream appears in both development and held-out roles")
    ...
    for stream_name, scene_group_id, frame_count in development_members:
        ...
        if stream_id in {_stream_id(item[0]) for item in heldout_members}:
            raise OcidEvaluationError(f"development stream resolves to held-out id: {stream_id}")
        ...
        if manifest.get("stream_id") != stream_id:
            raise OcidEvaluationError(f"manifest stream_id mismatch for {stream_id}")
        if metadata.get("role") != "development":
            raise OcidEvaluationError(f"manifest role mismatch for {stream_id}")
        if metadata.get("scene_group_id") != scene_group_id:
            raise OcidEvaluationError(f"manifest scene group mismatch for {stream_id}")
        frames = _list(manifest, "frames", f"manifest for {stream_id}")
        if len(frames) != frame_count:
            raise OcidEvaluationError(f"frame count mismatch for {stream_id}: expected {frame_count}, got {len(frames)}")
        for frame in frames:
            if not isinstance(frame, dict) or not isinstance(frame.get("image_path"), str):
                raise OcidEvaluationError(f"malformed frame entry in {stream_id}")
            _under(stream_directory, stream_directory / frame["image_path"], f"frame path in {stream_id}")
        if not annotation_path.is_file():
            raise OcidEvaluationError(f"missing annotation for {stream_id}")
...
def validate_development_input_pairs(
    ...
    *,
    ...
    """Require the exact canonical OCID evaluation development stream/annotation pairs.
    ...
    if len(stream_directories) != len(annotation_paths) or not stream_directories:
        raise OcidEvaluationError("supply one annotation for every stream")
    ...
    try:
        ...
        raise OcidEvaluationError(f"cannot resolve supplied development input: {error}") from error
    supplied_set = set(supplied)
    if len(supplied) != len(supplied_set):
        raise OcidEvaluationError("development stream/annotation pairs must be unique")
    if supplied_set != expected:
        ...
        raise OcidEvaluationError(
            "OCID evaluation requires exactly the canonical development inputs; "
...
def evaluate_profiles(
    ...
    if not isinstance(predictions_by_profile, Mapping) or not predictions_by_profile:
        raise OcidEvaluationError("predictions_by_profile must contain at least one profile")
    ...
    for profile_id in sorted(predictions_by_profile):
        if not isinstance(profile_id, str) or not profile_id:
            raise OcidEvaluationError("profile IDs must be non-empty strings")
        supplied = predictions_by_profile[profile_id]
        if not isinstance(supplied, Mapping) or set(supplied) != expected_ids:
            raise OcidEvaluationError(f"profile {profile_id!r} must supply exactly the development streams")
        ...
        for stream in streams:
            ...
            if not all(isinstance(item, PredictedCandidate) for item in predicted):
                raise OcidEvaluationError(f"profile {profile_id!r} has invalid candidates for {stream.stream_id}")
            metrics = {
    ...
    return {"schema_version": "ocid-development-results-1.0", "iou_grid": list(IOU_GRID), "profiles": profiles}
...
def _validate_groups(groups: list[Any], role: str) -> list[tuple[str, str, int]]:
    ...
    for group in groups:
        if not isinstance(group, dict):
            raise OcidEvaluationError(f"{role} group is not an object")
        ...
        if not isinstance(group_id, str) or not group_id or group_id in group_ids:
            raise OcidEvaluationError(f"duplicate or missing {role} scene_group_id")
        if isinstance(count, bool) or not isinstance(count, int) or count <= 0 or not isinstance(members, list) or not members:
            raise OcidEvaluationError(f"malformed {role} group {group_id!r}")
        group_ids.add(group_id)
        for member in members:
            if not isinstance(member, str) or not member or member in stream_names:
                raise OcidEvaluationError(f"duplicate or missing {role} stream")
...
def _validate_totals(expected: Mapping[str, Any], development: list[Any], heldout: list[Any]) -> None:
    ...
    for key, actual in checks.items():
        if expected.get(key) != actual:
            raise OcidEvaluationError(f"expected_totals.{key} mismatch: expected {expected.get(key)!r}, got {actual}")
...
def _validate_inventory_argument(streams: tuple[DevelopmentStream, ...]) -> None:
    if any(not isinstance(stream, DevelopmentStream) for stream in streams):
        raise OcidEvaluationError("inventory must contain DevelopmentStream values")
    if not streams or len({stream.stream_id for stream in streams}) != len(streams):
        raise OcidEvaluationError("inventory must contain unique development streams")
...
def _under(root: Path, path: Path, label: str) -> Path:
    ...
    try:
        ...
        raise OcidEvaluationError(f"{label} escapes reviewed root: {path}") from error
    return resolved
...
def _object(value: Mapping[str, Any], key: str, label: str) -> dict[str, Any]:
    ...
    if not isinstance(result, dict):
        raise OcidEvaluationError(f"{label}.{key} must be an object")
    return result
...
def _groups(roles: Mapping[str, Any], role: str) -> list[Any]:
    ...
    if not isinstance(value, list) or not value:
        raise OcidEvaluationError(f"roles.{role} must be a non-empty list")
    return value
...
def _list(value: Mapping[str, Any], key: str, label: str) -> list[Any]:
    ...
    if not isinstance(result, list):
        raise OcidEvaluationError(f"{label}.{key} must be a list")
    return result
